...
- [ ] [Stage 7] Optimize staged safety recovery with optional three-way merge drafts, checkpoint retention/cleanup, and short rollback history. Merge drafts start from the current Arduino file, require editor review and Save File before disk writes, retain Keep Arduino Version as the non-destructive default, and preserve the external-change guard.
